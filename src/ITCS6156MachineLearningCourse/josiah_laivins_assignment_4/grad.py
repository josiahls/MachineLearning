...
def steepest(x, gradf, f, *fargs, **params):
    ...
    while i <= nIterations:
        ...
        if max(abs(newx - x)) < xPrecision:
            return {'w': newx, 'f': newf, 'nIterations': i, 'wtrace': wtrace[:i, :] if wtracep else None,
                    'ftrace': ftrace[:i] if ftracep else None,
    ...
    return {'w': newx, 'f': newf, 'nIterations': i, 'wtrace': wtrace[:i, :] if wtracep else None,
            'ftrace': ftrace[:i] if ftracep else None, 'reason': "did not converge"}
